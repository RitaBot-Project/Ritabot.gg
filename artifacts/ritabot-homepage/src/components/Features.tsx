        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6">
          {features.map((feature, index) => {
            ...
            return (
              <div 
                key={index}
                className="group relative bg-card rounded-2xl md:rounded-3xl p-4 md:p-8 border border-border/50 dark:border-white/5 hover:border-border dark:hover:border-white/10 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-black/10 dark:hover:shadow-black/50 overflow-hidden"
              >
                ...
                <div className="flex items-center gap-3 mb-2 md:block">
                  <div className={`w-9 h-9 md:w-14 md:h-14 shrink-0 rounded-xl md:rounded-2xl ${feature.bg} flex items-center justify-center md:mb-6 border border-border/50 dark:border-white/5 group-hover:scale-110 transition-transform duration-300`}>
                    <Icon className={`w-4 h-4 md:w-7 md:h-7 ${feature.color}`} />
                  </div>
                  <h3 className="text-sm font-bold text-foreground md:hidden leading-tight">
                    {feature.title}
                  </h3>
                </div>

                <h3 className="hidden md:block text-xl font-bold text-foreground mb-3 font-display">
                  {feature.title}
                </h3>
