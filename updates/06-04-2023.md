...
As many of you have seen, there are a few version of RITA. So I wanted to quickly tell you what they are, why they exist and what it means for you.&#x20;

When we wanted to make a change to RITA, we had to take her offline, upload the new changes then reboot her, test and see if it impacts anything, if it did then we had to revert and try the process all over again. Not very efficient. \
\
So just like BITA, (our testing bot - not the same as global changes) we wanted to come up with a way to ensure reliability, allow us to continue to develop with minimal interference and provide a stable service to meet our uptime guarantees, thus the new family member. \
\
You can distinguish the difference by the Bot Icon, BLUE, YELLOW, RED and PURPLE.

* BLUE - This bot is no different from YELLOW in terms of core functionality. Updates may be a day or so behind YELLOW but this is because we will not interrupt service to BLUE users to reboot and install fixed (unless absolutely necessary). Automatic reboots takes 17 seconds on average and they happen each night at midnight GMT.
* RED - Our BITA bot, this BOT runs an experimental build, may have bugs and uptime is not guaranteed, you can not request acces at this time to use BITA and access is invite only. All new features are tested on BITA first to make sure they work and features, functions, commands and slash commands can be added and removed at any time.
* YELLOW - Our public bot, which is still in the all 18k servers from before we had to charge for our services. As such boot time is around 60 seconds on average and service can be disrupted through the day when we push bug fixes. With its large server base (although they are inactive) it generates a large amount of diagnostic data, allowing us to track down bugs and start to fix them.&#x20;
...
Any user that pays for RITA can swap to use RITA BLUE at any time, and we encourage and request you do so, this means we provide you the best service we can. You can invite RITA blue by clicking Here or clicking on RITA’s Icon (any of them) and clicking “Add to server”
